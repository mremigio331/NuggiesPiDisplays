...
def render(canvas, data: dict, show_pct: bool = True) -> None:
    ...
    direction_color = (
        COLOR_UP      if direction == "up"
        else COLOR_DOWN    if direction == "down"
        else COLOR_NEUTRAL
    )
    ...
    change_block_w = (TRIANGLE_W + TRIANGLE_GAP + _tws(change_str)) if change_str and not neutral else _tws(change_str)
    ...
    if USING_HARDWARE and _gfx and _hw_large and _hw_small:
        ...
        if price_str:
            _gfx.DrawText(canvas, _hw_small, MATRIX_W - _tws(price_str), ROW0_SMALL_Y, dc, price_str)
        ...
        if price_str:
            ...
            draw.text((MATRIX_W - pw, 0), price_str, font=_pil_small, fill=direction_color)

        draw.text((0, 9), short_label, font=_pil_small, fill=COLOR_LABEL)

        if change_str:
            cw = round(draw.textlength(change_str, font=_pil_small))
            draw.text((MATRIX_W - cw, 9), change_str, font=_pil_small, fill=direction_color)
