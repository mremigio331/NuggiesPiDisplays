...
function nbaPeriodLabel(game) {
  const { state, period, clock } = game;

  if (state === "in") {
    const p = period > 4 ? (period === 5 ? "OT" : `OT${period - 4}`) : `Q${period}`;
    return clock ? `${p} ${clock}` : p;
  }

  if (state === "post") {
    if (period === 5) return "Final/OT";
    if (period > 5) return `Final/OT${period - 4}`;
    return "Final";
  }
...
export default function NBAGameCard({ game, favoriteTeams, activeEventIds }) {
  const {
    away_team,
    home_team,
    away_score,
  ...
  const periodLabel = nbaPeriodLabel(game);
